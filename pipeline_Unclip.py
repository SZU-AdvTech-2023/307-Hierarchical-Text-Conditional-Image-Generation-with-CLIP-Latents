...
class StableUnCLIPPipeline_prior(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    """
    ...
    def __call__(
        ...
        # prior_text_encoder_hidden_states.register_hook(self.print_grad)
        # prior_prompt_embeds.register_hook(self.print_grad)
        ...
        # prior_latents.register_hook(self.print_grad)
        # latent_model_input.register_hook(self.print_grad)
        # predicted_image_embedding.register_hook(self.print_grad)
        prior_latents = self.prior.post_process_latents(prior_latents)
        # prior_latents.register_hook(self.print_grad)
        image_embeds = prior_latents
        # image_embeds.register_hook(self.print_grad)
        # done prior
...
class StableUnCLIPPipeline_prior_v2(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    """
    Pipeline for text-to-image generation using stable unCLIP.
    ...
    def __call__(
        ...
        return image_embeds, prior_prompt_embeds